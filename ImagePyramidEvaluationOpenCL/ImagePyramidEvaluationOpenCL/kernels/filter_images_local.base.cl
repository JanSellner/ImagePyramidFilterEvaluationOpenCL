 ...
 */
/* GENERATE_TYPE */ filter_sum_MULTIPLICITY_local_DERIV_NxN(read_only image2d_t imgIn/* GENERATE_REMOVE_PREDEFINED:,
                                   ...
                                   const int border)
{
    ...
    for (int y = yLocalId; y < yLocalSize + 2 * ROWS_HALF_NxN; y += yLocalSize)
    {
        for (int x = xLocalId; x < xLocalSize + 2 * COLS_HALF_NxN; x += xLocalSize)
        {
            ...
            int2 coordBorder = borderCoordinate((int2)(x - COLS_HALF_NxN + xBase, y - ROWS_HALF_NxN + yBase), rows, cols, border);
    ...
    coordBase = (int2)(xLocalId + COLS_HALF_NxN, yLocalId + ROWS_HALF_NxN);
    int2 coordCurrent;
    float color;
 ...
 */
kernel void filter_MULTIPLICITY_local_DERIV_NxN(read_only image2d_t imgIn,
                                    write_only image2d_t imgOut/* GENERATE_DOUBLE:1,
                                    write_only image2d_t imgOut2*//* GENERATE_REMOVE_PREDEFINED:,
                                    constant float* filterKernel/* GENERATE_DOUBLE:1,
                                    constant float* filterKernel2*/*/,
                                    const int filterRowsHalf, // GENERATE_REMOVE
                                    const int filterCols,     // GENERATE_REMOVE
