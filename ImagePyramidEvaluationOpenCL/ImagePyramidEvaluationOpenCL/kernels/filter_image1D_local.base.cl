 ...
 */
/* GENERATE_TYPE */ filter_sum_MULTIPLICITY_local_DERIV_NxN(read_only image1d_buffer_t imgIn,
                                                            ...
                                                            const int border)
{
    ...
    for (int y = yLocalId; y < yLocalSize + 2 * ROWS_HALF_NxN; y += yLocalSize)
    {
        for (int x = xLocalId; x < xLocalSize + 2 * COLS_HALF_NxN; x += xLocalSize)
        {
            ...
            int3 coordBorder = borderCoordinate((int3)(x - COLS_HALF_NxN + xBase, y - ROWS_HALF_NxN + yBase, coordBase.z), rows, cols, border);
            localBuffer[y * LOCAL_SIZE_COLS_NxN + x] = readValue1D(imgIn,  locationLookup, coordBorder.z, coordBorder.x, coordBorder.y);   // Fill local buffer
        }
    }
#else
    /*
    ...
    coordBase = (int3)(xLocalId + COLS_HALF_NxN, yLocalId + ROWS_HALF_NxN, 0);
    int2 coordCurrent;
    float color;
 ...
 */
kernel void filter_MULTIPLICITY_local_DERIV_NxN(read_only image1d_buffer_t imgIn,
                                                constant struct Lookup* locationLookup,
                                                write_only image1d_buffer_t imgOut/* GENERATE_DOUBLE:1,
                                                write_only image1d_buffer_t imgOut2*//* GENERATE_REMOVE_PREDEFINED:,
                                                constant float* filterKernel/* GENERATE_DOUBLE:1,
                                                constant float* filterKernel2*/*/,
                                                const int filterRowsHalf, // GENERATE_REMOVE
                                                const int filterCols,     // GENERATE_REMOVE
                                                ...
                                                const int border)
{
    ...
    /* GENERATE_TYPE */ sum = filter_sum_MULTIPLICITY_local_DERIV_NxN(imgIn, locationLookup/* GENERATE_REMOVE_PREDEFINED:, filterKernel/* GENERATE_DOUBLE:1, filterKernel2*/*/,/* GENERATE_REMOVE: filterRowsHalf, filterCols, filterColsHalf,*/ coordBase, border);

    writeValue1D(imgOut/* GENERATE_DOUBLE:1*/, locationLookup, coordBase.z, coordBase.x, coordBase.y, sum/* GENERATE_DOUBLE:.x*/);
    /* GENERATE_DOUBLE:writeValue1D(imgOut2, locationLookup, coordBase.z, coordBase.x, coordBase.y, sum.y);*/
}
